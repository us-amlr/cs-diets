setwd("C:/Users/fanny.sanchez/Desktop/cs-diets")
read.csv("diets.scat.csv")
library(tidyverse)


diets <- read.csv("diets.scat.csv")

diets.scat <- diets %>%  filter(sample_type %in% c("scat", "Scat"))
scat.diet.plot <- diets.scat %>% pivot_longer(cols = krill_type:squid_type,
                                              names_to = "Species",
                                              values_to = "Presence") %>%
...
#Do a proportion plot for seasons to visualize better
...
ggplot(diets.scat, aes(x = season_name)) +   geom_bar(stat = "identity")

xy <- ggplot(diets.scat, aes(x = season_name)) + geom_bar(fill = "coral4") 
